...
3. **내용 반영**:
   ...
   - 사용자의 현재 구간(노동자/숙련자/실력자/전문가/시스템/브랜드)을 PDF의 6단계 정의에 맞춰 분석하세요
...

{
  ...
  "frequent_thoughts": "이 구간에서 자주 하는 생각을 PDF 작성자의 언어 스타일로 작성하세요. 단정적이고 확신에 찬 톤('~이다', '~다')을 사용하고, 질문 형식('~는 무엇일까?', '~는?')을 활용하세요. PDF의 '학습자본' 개념과 '6단계 구간' 정의를 참고하여, 사용자가 현재 구간에서 일반적으로 하는 생각을 설명하세요. 왜 그런 생각을 하게 되는지 이유와 함께 설명하되, PDF 작성자가 직접 말하는 것처럼 작성하세요.",
  "unknown_things": "이 구간에서 잘 모르는 것들을 PDF 작성자의 언어 스타일로 설명하세요. '~라는 것', '~라는 점', '~라는 사실' 같은 표현을 사용하고, 단정적 톤('~이다', '~다')을 유지하세요. PDF의 '학습자본 vs 금융자본 vs 사회자본' 비교와 '구간별 축적해야 하는 학습자산' 내용을 참고하여, 사용자가 모르고 있는 핵심 개념을 설명하세요. 왜 모르게 되는지, 그리고 그것을 알아야 하는 이유를 포함하되, PDF 작성자가 직접 설명하는 것처럼 작성하세요.",
  "must_learn": "반드시 배워야 하는 것들을 PDF 작성자의 언어 스타일로 구체적으로 제시하세요. '반드시', '절대', '~해야 한다', '~해야 함' 같은 강조 표현과 의무 표현을 사용하세요. PDF의 '학습자본' 개념과 '새로운 돈의 정의'를 참고하여, 학습 자본가로 성장하기 위해 필요한 학습 내용을 제시하세요. 왜 배워야 하는지, 어떤 효과가 있는지 설명하되, PDF 작성자가 직접 조언하는 것처럼 작성하세요.",
  "recommended_training": "추천하는 생각 훈련을 PDF 작성자의 언어 스타일로 제시하세요. '~해보세요', '~할 것', '~이다' 같은 친근하면서도 단정적인 표현을 사용하세요. PDF의 '학습 자본가의 특징'과 '부학부 빈학빈' 개념을 참고하여, 배우는 감각을 복리 개념으로 업데이트하는 훈련 방법을 제시하세요. 구체적인 방법과 함께 왜 이것이 필요한지 설명하되, PDF 작성자가 직접 추천하는 것처럼 작성하세요.",
  "avoid_studies": "지금은 피해야 할 공부를 PDF 작성자의 언어 스타일로 제시하세요. '~는 ~가 아니라' 같은 비교/대조 표현을 활용하고, 단정적 톤('~이다', '~다')을 유지하세요. PDF의 '기존의 학습 vs 학습 자본가의 학습' 비교를 참고하여, 정답을 맞히는데 편향된 학습을 피하고, 자신이 만들고 싶은 세계를 만들어가기 위한 학습으로 전환해야 함을 설명하세요. 왜 피해야 하는지, 대신 무엇을 해야 하는지 설명하되, PDF 작성자가 직접 경고하는 것처럼 작성하세요."
}
